...
impl FrameParser {
    ...
    pub fn par_start(&mut self, demo_bytes: &[u8]) -> Result<Vec<StartEndOffset>, DemoParserError> {
        let b = Instant::now();
        let start_pos = FrameParser::split_file_into_n_chunks(demo_bytes.len(), 4);
        ...
        outputs.push(StartEndOffset {
            start: offsets.last().cloned().unwrap_or(16),
            end: demo_bytes.len(),
        });
        offsets.sort();
        offsets.dedup();
        println!("part 2 took {:?}", b.elapsed());

        if FrameParser::safety_check(&outputs) {
            Ok(outputs)
        } else {
            Ok(vec![StartEndOffset {
                start: 16,
                end: demo_bytes.len(),
            }])
        }
    }
    fn safety_check(sorted_offsets: &[StartEndOffset]) -> bool {
        // checks that all bytes are parsed
