...
pub struct Field5 {
    ...
    pub children: Vec<Option<Decoder>>,
...
impl Field5 {
    pub fn get_child(&self, idx: usize) -> &Field5 {
    ...
    pub fn find_decoder(&mut self, model: FieldModel, qf_map: &mut QfMapper) {
        ...
        match model {
            ...
            FieldModelVariableArray => {
                self.children.push(Some(Decoder::UnsignedDecoder));
                // self.serializer = self.base_decoder = Some(Decoder::UnsignedDecoder);
                ...
                self.children.push(self.child_decoder);
            }
            FieldModelNOTSET => panic!("Field model not set??"),
        }
        if bef == self.decoder && model != FieldModelVariableArray {
            println!("{:#?} {:?}", self, model);
        }
    }
    ...
    pub fn decoder_from_path(&self, path: &FieldPath, pos: usize, parse_inventory: bool) -> FieldInfo {
        match self.model {
            FieldModelSimple => {
                ...
                // println!("{:?} => {:?}", self.category, self.model);
                return FieldInfo {
                    decoder: self.decoder,
                    should_parse: self.should_parse,
            ...
            FieldModelVariableTable => {
                if path.last >= pos + 1 {
                    // println!("{:?} => {:?}variabletable1", self.category, self.model);
                    match &self.serializer {
                        Some(ser) => {
                    ...
                    // println!("{:?} => {:?}variabletable2", self.category, self.model);
                    return FieldInfo {
                        decoder: self.base_decoder.unwrap(),
                        should_parse: self.should_parse,
                        prop_id: self.prop_id as u32,
...
impl Parser {
    ...
    /*
    pub fn parse_sendtable_new(&mut self, tables: CDemoSendTables) -> Result<(), DemoParserError> {
        let mut bitreader = Bitreader::new(tables.data());
        let n_bytes = bitreader.read_varint()?;
        let bytes = bitreader.read_n_bytes(n_bytes as usize)?;
        let serializer_msg: CSVCMsg_FlattenedSerializer = Message::parse_from_bytes(&bytes).unwrap();
        ...
        let mut fields: Vec<Option<Field>> = vec![None; serializer_msg.fields.len()];
        let mut prop_controller = PropController::new(
            self.wanted_player_props.clone(),
            self.wanted_other_props.clone(),
            self.real_name_to_og_name.clone(),
        ...
        for serializer in serializer_msg.serializers.iter() {
            ...
            let fields_v: Vec<Field> = Vec::with_capacity(serializer.fields_index.len());
            let field_names: Vec<String> = Vec::with_capacity(serializer.fields_index.len());

            for idx in 0..fields.len() {
                let fi = serializer.fields_index[idx];

                if let Some(inner) = fields[fi as usize]{
                    inner.fi
                }


                if fields[fi as usize].is_none(){
                    fields[fi as usize]. =
                }
            }

            let ser_len = serializer.fields_index.len();
            println!("{:?} {:?}", my_serializer.name, ser_len);
        }
        // println!("{:#?}", fields);
        Ok(())
    }
    */
    ...
    pub fn create_field(fieldData: Vec<Option<Field>>) {}

    pub fn find_type(field: &mut Field) {
        let is_pointer = Parser::is_pointer(&field);
        let is_array = Parser::is_array(&field);
        let is_vector = Parser::is_vector(&field);

        if is_pointer {
            field.category = FieldCategory::Pointer;
        } else if is_vector {
            field.category = FieldCategory::Vector;
        } else if is_array {
            field.category = FieldCategory::Array;
        } else {
            field.category = FieldCategory::Value;
        }
    }
    pub fn is_pointer(field: &Field) -> bool {
...
pub fn field_from_msg(field: &ProtoFlattenedSerializerField_t, serializer_msg: &CSVCMsg_FlattenedSerializer) -> Field {
